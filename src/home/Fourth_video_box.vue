  ...
  methods: {
    ajax(){
      this.$axios.get("http://api.paopao.vip/strategy/video").then(res=>{
        ...
        let len = videoItem.length > this.pageSize ? this.pageSize : videoItem.length
    ...
    // 点击分页按钮的时候，改变视图
     changeView(current){
      ...
      let end = (current*this.pageSize > this.videoItem.length) ? this.videoItem. length : current*this.pageSize
